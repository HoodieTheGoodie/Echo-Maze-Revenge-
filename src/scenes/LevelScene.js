...
export class LevelScene extends Phaser.Scene {
    ...
    update(_, delta) {
        ...
        this.player.x = Phaser.Math.Clamp(this.player.x, this.arenaBounds.left + this.playerRadius, this.arenaBounds.right - this.playerRadius);
        this.player.y = Phaser.Math.Clamp(this.player.y, this.arenaBounds.top + this.playerRadius, this.arenaBounds.bottom - this.playerRadius);
    }
    spawnRoomFrame() {
        const { width, height } = this.scale;
        const roomBounds = new Phaser.Geom.Rectangle(45, 70, width - 90, height - 120);
        this.arenaBounds = roomBounds;
        const room = this.add.rectangle(roomBounds.centerX, roomBounds.centerY, roomBounds.width, roomBounds.height, 0x0b1322, 0.3);
        room.setStrokeStyle(3, 0x00e1ff, 0.65);
    }
    generateConnectedWallsAndGetPlayerSpawn() {
        ...
        this.arenaBounds = new Phaser.Geom.Rectangle(this.gridOriginX, this.gridOriginY, gridPixelWidth, gridPixelHeight);
    ...
    findClosestFreeCellToCenter(centerCell) {
    ...
    spawnPlaceholderEnemies(playerSpawn) {
    ...
    movePlayerAndResolveCollisions(deltaX, deltaY) {
